...
import randomforest.RandomForestDataSet;
...
public class RandomForest {
    ...
    public static void fit(DoubleDataSet samples, IntegerDataSet classification, FitConfig config, int batchSize) {
        ...
        int numBatchesFloor = numEstimadors/batchSize;
        int numBatchesCeil = (int)Math.ceil((double)numEstimadors/(double)batchSize);
        ...
        System.out.println("______ numBatchesFloor:" + Integer.toString(numBatchesFloor));
        System.out.println("______ numBatchesCeil:" + Integer.toString(numBatchesCeil));
        
        for (int i = 0; i < numBatchesFloor; i++) {
            long batchSeed = rd.nextLong();
            for(int j = 1; j < batchSize; j++){
                rd.nextLong();
            }
            generateTaskBatch(batchSeed, treeFitConfig, numSamples, samples, classification, config, batchSize, i);
        }
        if(numBatchesFloor != numBatchesCeil){
            long batchSeed = rd.nextLong();
            generateTaskBatch(batchSeed, treeFitConfig, numSamples, samples, classification, config, numEstimadors%batchSize, numBatchesFloor);
        }
        COMPSs.barrier();
    }

    public static void generateTaskBatch(long batchSeed, TreeFitConfig treeFitConfig, int numSamples, DoubleDataSet samples, IntegerDataSet classification, FitConfig config, int batchSize, int numeroTasca) {
        Random rd = new Random(batchSeed); 
        System.out.println("______ tasca batch " + numeroTasca);
        for (int estimatorId = 0; estimatorId < batchSize; estimatorId++) {
            long randomSeed = rd.nextLong();
            IntegerDataSet sampleSelection = randomSelection(0, numSamples, numSamples, randomSeed, numeroTasca);
            TreeTrainer.trainTreeWithDataset(samples, classification, sampleSelection, treeFitConfig, randomSeed, numeroTasca);
            // COMPSs.deregisterObject(sampleSelection);
        }
        // for (Tree tree : trees) {
        //     COMPSs.deregisterObject(tree);
        // }
    }
